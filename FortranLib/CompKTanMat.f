      subroutine CompKTanMat(Fn1,Fn,Fvn,Fpn,dt,bmax,Qn,MP,
     & Cauchy,Epsi,D)
c***************************************************************************
! This subroutine evaluate Kirchhoff derivative in relation to logarithm Strain based on
...
c***************************************************************************
     ...
     &  Qn1, Fpn1(3,3), Fvn1(3,3), deltaQ, nor,CSv(6),Ev(6)		 
	  ...
	  DOUBLE PRECISION Fpr(3,3),FpnInv(3,3),small,V(3,3)
	  DOUBLE PRECISION Cpr(3,3),Mpr(3,3),cprv(3), epr(3), Betab,Taub
	  DOUBLE PRECISION q0(3), deltaQc, ee_p(3), deltaQqj(3),e(3)
	  ...
	  DOUBLE PRECISION Cau2, eigVec(3,3), eigVal(3), Jn1, incn,El(3,3)
...
! for t=1 -> Forward differeces, t=2 -> Central diferences
      ...
      if (t .eq. 1) then
         do i=1,6
		    dCS(i,:)=dCSp(i,:)-CSv
			call vec2mat(dEpsip(i,:),El)
			call eigsym33(El,V,e)
			Jn1=exp(e(1))*exp(e(2))*exp(e(3))
			dEpsi(i,:)=dEpsip(i,:)-Ev
		    D(i,:)=Jn1*dCS(i,:)/inc
         end do
      else
